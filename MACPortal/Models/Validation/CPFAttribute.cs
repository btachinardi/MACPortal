...
namespace MACPortal.Models.Validation
{
    public class CPFAttribute : ValidationAttribute
    {
        ...
        {
            if (value == null) return new ValidationResult(FormatCPFErrorMessage(validationContext));
            if (Validate(value.ToString()))
            {
                return ValidationResult.Success;
            }
            return new ValidationResult(FormatCPFErrorMessage(validationContext));
        }

        public string FormatCPFErrorMessage(ValidationContext validationContext)
        {
            var currentValue = validationContext.ObjectType.GetProperty("Name").GetValue(validationContext.ObjectInstance);
            return String.Format(CultureInfo.CurrentCulture, ErrorMessageString, new { currentValue });
        }
