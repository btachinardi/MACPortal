...
namespace MACPortal.Controllers
{
    public class AccountController : BootstrapBaseController
    {
        ...
        public ActionResult ForgotPassword(ForgotPasswordModel model)
        {
            ...
            {
                var employee = db.Employees.AsEnumerable().First(e => e.UserID == WebSecurity.GetUserId(model.UserName));
                ...
                mailer.ForgotPassword(email, encryptedToken, user.UserID, Server.MapPath("~/Content/images/logo-with-products-smaller.png")).Send();
                db.SaveChanges();
        ...
        public ActionResult ChangePassword(string recoverPasswordKey, int userID)
        {
            ...
            if (employee.User.RecoverPasswordToken != token)
            {
                Error("Código para recuperação de senha inválido.");
                return RedirectToAction("ForgotPassword");
            }
            if (employee.User.RecoverPasswordExpiration < DateTime.Now)
            {

                Error("Este código para recuperação de senha expirou.");
                return RedirectToAction("ForgotPassword");
            }
            return View(new ChangePasswordModel { Token = recoverPasswordKey, UserName = employee.User.UserName });
        }
        ...
        public ActionResult Manage(LocalPasswordModel model)
        {
            ...
            {
                // Employee does not have a local password so remove any validation errors caused by a missing
                // OldPassword field
        ...
        private static string ErrorCodeToString(MembershipCreateStatus createStatus)
        {
            ...
            switch (createStatus)
            {
                case MembershipCreateStatus.DuplicateUserName:
                    return "Employee name already exists. Please enter a different user name.";
