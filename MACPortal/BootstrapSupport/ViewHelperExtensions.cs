...
namespace BootstrapSupport
{
    public static class DefaultScaffoldingExtensions
    {
        ...
        public static PropertyInfo[] VisibleProperties(this IEnumerable Model, bool hideId = false)
        {
            ...
            return elementType.GetProperties().Where(info => !hideId || info.Name != elementType.IdentifierPropertyName()).OrderedByDisplayAttr().ToArray();
        }

        public static PropertyInfo[] VisibleProperties(this Object model, bool hideId = false)
        {
            return ObjectContext.GetObjectType(model.GetType()).GetProperties().Where(info => !hideId || info.Name != model.IdentifierPropertyName()).OrderedByDisplayAttr().ToArray();
        }
        ...
        {
            var identifierPropertyName = model.IdentifierPropertyName();
            var property = model.GetType().GetProperty(identifierPropertyName);
            return property.GetValue(model, new object[0]);
        }
        ...
        public static string IdentifierPropertyName(this Type type)
        {
            ...
            else if (type.GetProperties().Any(p => p.Name.Equals(type.Name + "id", StringComparison.CurrentCultureIgnoreCase)))
            {
                var property = type.GetProperties().First(
                    p => p.Name.Equals(type.Name + "id", StringComparison.CurrentCultureIgnoreCase));
                return property.Name;
            }
            else if (type.GetProperties().Any(p => p.Name.Equals(ObjectContext.GetObjectType(type).Name + "id", StringComparison.CurrentCultureIgnoreCase)))
            {
                var property = type.GetProperties().First(
                    p => p.Name.Equals(ObjectContext.GetObjectType(type).Name + "id", StringComparison.CurrentCultureIgnoreCase));
                return property.Name;
            }
            return "";
        }
